-- 'forecast'/'campaign' (resource_type) were added in 0049 — separate
-- migration for the same reason as every prior enum-value + seed pair
-- (0009/0010, ..., 0046/0048): Postgres won't let a new enum value be
-- used in the same transaction that adds it.
--
-- Same "shared sales-team working set" tier as the rest of Phase 6:
-- owner/admin/member all get full CRUD, viewer stays read-only.

insert into permissions (org_id, role, resource_type, action)
select null::uuid, r.role, rt.resource_type, a.action
from unnest(array['forecast', 'campaign']::resource_type[]) as rt(resource_type)
