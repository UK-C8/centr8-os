...
export async function GET(req: NextRequest) {
  try {
    ...
    const rows = await withOrgContext(userId, async (db) => {
      await requirePermission(db, userId, orgId, "engagement", "read");
...
export async function POST(req: NextRequest) {
  try {
    ...
    const [row] = await withOrgContext(userId, async (db) => {
      await requirePermission(db, userId, body.org_id, "engagement", "create");
      return db
        .insert(engagementSurveys)
        .values({ orgId: body.org_id, title: body.title, questions: body.questions ?? [] })
        .returning();
    });
